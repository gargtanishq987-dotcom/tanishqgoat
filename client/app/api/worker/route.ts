...
import {
  ...
  incrementAnalytics, getAllInboxes, resetDailySentCounts, getSettings,
  getMessagesByLead,
...
export async function POST(req: NextRequest) {
  ...
  const results = { sent: 0, followups: 0, errors: 0, skipped: 0, sameInboxValidated: 0 };
...
async function processInitialEmail(lead: Lead, results: Record<string, number>): Promise<void> {
  ...
  if (nextFollowUpAt) {
    await logEvent({
      ...
      metadata: { followUpIndex: 1, scheduledAt: nextFollowUpAt },
    });
  }
...
async function processFollowUp(lead: Lead, results: Record<string, number>): Promise<void> {
  ...
  if (!inbox) {
    await logEvent({
      leadId: lead.id, campaignId: lead.campaignId, inboxId: lead.inboxId,
      type: "FOLLOWUP_FAILED",
      metadata: { error: `Original sending inbox ${lead.inboxId} no longer exists` },
    });
    results.skipped++;
    return;
  }

  // ✓ Validate: follow-up MUST go from the same inbox as the initial email
  // lead.inboxId stores the inbox that sent the first email — we already use it above.
  // This is the only inbox we will use; we never switch inboxes mid-sequence.
  results.sameInboxValidated = (results.sameInboxValidated ?? 0) + 1;
  ...
  const nextFollowUpAt = delayDays && nextFollowUpBody
    ? now + delayDays * 86400_000
    : null;
